...
def _empty_outputs():
    return {
        ...
        s1_status: "Waiting to run stage 1.",
...
def run_pipeline(video_path):
    ...
    yield {
        **_empty_outputs(),
        s1_panel: gr.update(visible=True),
        s1_status: "⏳ Running stage 1 screening with MobileNet + YOLO...",
    }
    ...
    s1_outputs = {
        **_empty_outputs(),
        s1_panel: gr.update(visible=True),
        s1_status: f"{'✅ PASSED' if s1['status'] == 'pass' else '❌ STOPPED'} — {s1['message']}",
    ...
    yield {
        **s1_outputs,
        s2_panel: gr.update(visible=True),
        s2_status: "⏳ Running stage 2 risk grading + Grad-CAM...",
    }
...
with gr.Blocks(
    ...
    with gr.Column(visible=True, elem_classes=["result-card"]) as s1_panel:
        ...
        s1_video = gr.Video(label="Stage 1 Annotated Video", visible=False)
    ...
    with gr.Column(visible=True, elem_classes=["result-card"]) as s2_panel:
        ...
        with gr.Row(equal_height=True):
            ...
            with gr.Column(scale=1):
                s2_gradcam_video = gr.Video(label="Grad-CAM Comparison", visible=False)
