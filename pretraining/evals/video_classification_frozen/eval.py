...
def main(args_eval, resume_preempt=False):
    ...
    layer_decay = args_opt.get("layer_decay", 0.75)  # 【全量微调新增】支持在config中配置layer_decay
    opt_kwargs = [
        dict(
    ...
    folder = os.path.join(pretrain_folder, "video_classification_finetuned/") # 【修改】更换目录防止覆盖
    ...
    # 【修复 DDP 报错】必须在包裹 DDP 之前解冻基座参数
    encoder.requires_grad_(True)
    
    # 【全量微调新增】给 encoder 包上 DDP
    encoder = DistributedDataParallel(encoder, static_graph=True)

    # -- init classifier
    classifiers = [
        AttentiveClassifier(
            embed_dim=encoder.module.embed_dim if hasattr(encoder, 'module') else encoder.embed_dim,
            num_heads=num_heads,
    ...
    # -- optimizer and scheduler 【全量微调修改】传入 encoder 和 layer_decay
    ...
    # -- load training checkpoint 【全量微调修改】传入 encoder
    start_epoch = 0
    ...
    def save_checkpoint(epoch):
        ...
        save_dict = {
            "encoder": encoder.state_dict(),  # 【全量微调新增】保存更新后的基座权重
        ...
        if rank == 0:
            ...
            # 【新增】保存特定 epoch 的副本
            epoch_path = os.path.join(folder, f"checkpoint_epoch_{epoch}.pt")
...
def run_one_epoch(
    ...
    # 【全量微调新增】同步 encoder 的 train/eval 模式
    ...
    for itr, data in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_bfloat16):
            ...
            # 【全量微调修改】根据 training 状态决定是否保留计算图
            if not training:
                with torch.no_grad():
...
# 【全量微调新增】用于解析ViT层数的辅助函数 (针对 3D/视频模型同样适用)
def get_layer_id_for_vit(name, num_layers):
    if name.startswith("module."):
        name = name[7:]
    if name.startswith("patch_embed") or name.startswith("tubelet_embed") or name == "cls_token" or name == "pos_embed":
        return 0
...
# 【全量微调重写】带有 LLRD 和 No-Weight-Decay 的优化器
def init_opt(encoder, classifiers, iterations_per_epoch, opt_kwargs, num_epochs, use_bfloat16=False, layer_decay=0.75):
    ...
    # 动态计算 Transformer 层数
    layer_names = [n for n, p in encoder.named_parameters()]
    block_indices = [
        int(n.split('blocks.')[1].split('.')[0]) 
        for n in layer_names if "blocks." in n and n.split('blocks.')[1].split('.')[0].isdigit()
    ]
    num_layers = max(block_indices) + 1 if block_indices else 12
    classifier_layer_id = num_layers + 2 

    param_groups_dict = {}

    def add_param_to_group(name, p, layer_id):
        if not p.requires_grad:
            return
        # LLRD + No Weight Decay 对 Bias/Norm 等 1D 参数禁用正则化
        has_decay = False if len(p.shape) <= 1 else True
        key = (layer_id, has_decay)
        if key not in param_groups_dict:
            param_groups_dict[key] = []
        param_groups_dict[key].append(p)

    for n, p in encoder.named_parameters():
        layer_id = get_layer_id_for_vit(n, num_layers)
        add_param_to_group(n, p, layer_id)

    for n, p in c.named_parameters():
        add_param_to_group(n, p, classifier_layer_id)

    optimizer_grouped_parameters = []
    base_start_lr = kwargs.get("start_lr")
    base_ref_lr = kwargs.get("ref_lr")
    base_final_lr = kwargs.get("final_lr")
    base_ref_wd = kwargs.get("ref_wd")
    base_final_wd = kwargs.get("final_wd")

    for (layer_id, has_decay), params in param_groups_dict.items():
        decay_power = classifier_layer_id - layer_id
        decay_factor = layer_decay ** decay_power
        
        ref_wd = base_ref_wd if has_decay else 0.0
        final_wd = base_final_wd if has_decay else 0.0
        
        optimizer_grouped_parameters.append({
            "params": params,
            "mc_warmup_steps": int(kwargs.get("warmup") * iterations_per_epoch),
            "mc_start_lr": base_start_lr * decay_factor,
            "mc_ref_lr": base_ref_lr * decay_factor,
            "mc_final_lr": base_final_lr * decay_factor,
            "mc_ref_wd": ref_wd,
            "mc_final_wd": final_wd,
            "weight_decay": ref_wd,
        })

    logger.info(f"Using AdamW with LLRD (decay={layer_decay}) & No-Decay for Bias/Norm")
