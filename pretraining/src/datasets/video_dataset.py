...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for data_path in self.data_paths:
            ...
            if data_path[-4:] == ".csv":
                # === 彻底替换掉 Pandas，使用原生 Python 安全读取 ===
                current_count = 0
                # utf-8-sig 专门克制 Windows/Excel 偷偷加的隐藏字符 (BOM)
                with open(data_path, 'r', encoding='utf-8-sig') as f:
                    for line in f:
                        line = line.strip()  # 扒掉所有看不见的回车、换行、尾部空格
                        if not line:
                            continue
                ...
                self.num_samples_per_dataset.append(current_count)
                # ==================================================
