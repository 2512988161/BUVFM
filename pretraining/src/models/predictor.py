...
class Token_nmODE_Evolver(nn.Module):
    def __init__(self):
        super().__init__()
        self.gamma = None
    ...
    def forward(self, t, p):
        # -p + sin^2(p + gamma)
        return -p + torch.pow(torch.sin(p + self.gamma), 2)

class nmODE_Activation(nn.Module):
    def __init__(self):
        super().__init__()
        self.evolver = Token_nmODE_Evolver()
        self.register_buffer('integration_time', torch.tensor([0.0, 1.0]).float())

        self.scale = nn.Parameter(torch.tensor([0.1]))

    def forward(self, x):
        self.evolver.set_gamma(x)
        
        t = self.integration_time.to(x.device).type_as(x)
